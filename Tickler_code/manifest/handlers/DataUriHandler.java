...
public class DataUriHandler {
	...
	private ArrayList<DataUri> createDUsForSchemes(){
		...
		for (Map<String,String> h: this.collection){
			if (h.get("scheme")!= null){			
				/////Should work when components are combined
				DataUri d = this.getDataUriFromMap(h);
				
				// If the hashmap of the DataURI tag is complete (scheme and host) then don't add it to mishmash
	...
	private void addMimeTypeOnly(){
		if (this.isMimeTypeOnly()){
			//1- add the mimeTypes to totalDU
	...
	private void updateTotalDU(String compName, Map<String,String> h){
		...
		for (DataUri d:this.totalDU)
		{
			...
			//If the dataUri already has a value for this component, then add the old DataUri and the updated one
			if (!this.isDataUriCompEmpty(compName, d)){
				tempDataUri.add(d);
			// If There is a path conflict, then emtpy path values of new d2 before adding the new Path value
